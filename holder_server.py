...
async def get_tokens():
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute('''
                SELECT m.*, r.top10_percent, r.top100_percent, r.vol_24h, r.oi_usd, r.timestamp as last_update
                FROM monitored_tokens m
                LEFT JOIN (
                    SELECT symbol, top10_percent, top100_percent, vol_24h, oi_usd, timestamp,
...
async def get_history(symbol: str, hours: int = 48):
    try:
        ...
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT timestamp, top10_percent, top100_percent, vol_24h, oi_usd, price_index_info
                FROM holder_records

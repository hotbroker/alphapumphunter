...
class PatternConfig:
    ...
    consolidation_hours: int = 5
    volume_baseline_hours: int = 6
    min_volume_spike_multiple: float = 3.0
    min_volume_spike_quote: float = 1_000_000.0
    min_rise_pct: float = 25.0
    max_runup_multiple: float = 2.5
    min_pullback_pct: float = 30.0
    max_consolidation_range_pct: float = 12.0
    max_consolidation_close_drift_pct: float = 6.0
...
async def send_feishu_alert(webhook: str, message: str) -> None:
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        response.raise_for_status()
    logger.info("Feishu alert delivered")
    ...
    *,
    ...
    for pattern in patterns:
        if not should_notify(pattern, history, cooldown_seconds):
            ...
            continue
        await send_feishu_alert(webhook, format_pattern(pattern))
...
def add_common_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--min-quote-volume", type=float, default=100_000_000.0, help="Minimum 24h quote volume")
    parser.add_argument("--lookback-hours", type=int, default=60, help="Number of closed 1h candles to inspect")
    parser.add_argument("--consolidation-hours", type=int, default=5, help="Number of latest closed 1h candles for consolidation")
    parser.add_argument("--volume-baseline-hours", type=int, default=6, help="Hours used for pre-spike volume median")
    parser.add_argument("--min-volume-spike-multiple", type=float, default=3.0, help="Minimum volume spike / baseline")
    parser.add_argument("--min-volume-spike-quote", type=float, default=1_000_000.0, help="Minimum 1h spike quote volume")
    parser.add_argument("--min-rise-pct", type=float, default=25.0, help="Minimum rise from volume start to peak")
    parser.add_argument("--max-runup-multiple", type=float, default=2.5, help="Maximum rise multiple from volume start to peak")
    parser.add_argument("--min-pullback-pct", type=float, default=30.0, help="Minimum peak-to-low pullback")
    parser.add_argument("--max-consolidation-range-pct", type=float, default=12.0, help="Maximum high-low range in latest window")
    parser.add_argument("--max-consolidation-close-drift-pct", type=float, default=6.0, help="Maximum first-open to last-close drift")
    parser.add_argument("--concurrency", type=int, default=8, help="Concurrent Binance K-line requests")
    parser.add_argument("--webhook", default=FEISHU_WEBHOOK, help="Feishu bot webhook")
    parser.add_argument("--state-file", default=DEFAULT_STATE_FILE, help="Alert deduplication state JSON")
    parser.add_argument("--cooldown-minutes", type=int, default=1440, help="Cooldown for the same signal")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_parser.add_argument("--interval", type=int, default=300, help="Polling interval in seconds")
    return parser

...
KOL_SOURCE = configure_source("top_pump_energy")
...
async def compute_energy(symbol_usdt: str) -> Dict[str, Any]:
    ...
    price_filter = setting(KOL_SOURCE, "price_move_filter_pct", 9.0)
    if price_change_pct < price_filter:
        out.update({
            "energy_level": 0,
            "energy_note": f"价格波动不足{price_filter:g}%",
            "vol_usd_list": [],
            "taker_buy_ratio": [],
        })
    ...
    min_any_volume = setting(KOL_SOURCE, "min_any_15m_quote_volume", 1_000_000.0)
    if max(vol_usd_list) < min_any_volume:
        out.update({
            "energy_level": 0,
    ...
    energy_trigger = setting(KOL_SOURCE, "energy_trigger_quote_volume", 6_000_000.0)
    energy_ratio = setting(KOL_SOURCE, "energy_volume_ratio", 8.0)
    good1 = last_max > energy_trigger and (average_ref > 0 and last_max / average_ref > energy_ratio)
    energy_level = 0
    energy_note = ""
    if good1:
        energy_level = 2
        energy_note = "能量不错" #800w 超8倍
        if last_max > setting(KOL_SOURCE, "energy_level3_quote_volume", 20_000_000.0):
            energy_level = 3
            energy_note = "能量波动相当好"
    else:
        energy_level = 1
        ...
        try:
            ...
            for i, r in enumerate(filtered[: limit], 1):
                ...
                if notify:
                    ...
                    alert_level = setting(KOL_SOURCE, "alert_min_energy_level", 2)
                    alert_volume = setting(KOL_SOURCE, "alert_min_recent_quote_volume", 8_000_000.0)
                    if vol_last5 and energy_level >= alert_level and max(vol_last5) > alert_volume:
                        prev = last_alert_ts.get(sym, 0)
                        if now_ts - prev >= cooldown_secs:
                            last_alert_ts[sym] = now_ts
                            success = False

                            logger.debug("KOL source skips trading and position reporting")
                            ...
                            title="补充其它币种提示\n"
                            if utils.is_goodpump(vol_last5, ratios)  and qv and qv>3000*10000:
                                title=f"补充其它币种提示 🔥🔥推荐：{_base_from_symbol(sym)}\n"
                            emit_signal(
                                symbol=_base_from_symbol(sym),
                                source="kol_toppump.py",
                                indicator="top_pump_energy",
                                direction="LONG",
                                price=last_price,
                                summary=(
                                    f"15分钟能量等级 L{energy_level}，"
                                    f"最近2小时波动 {price_change_pct2:.2f}%，24小时涨幅 {pct24:.2f}%"
                                ),
                                details={
                                    "energy_level": energy_level,
                                    "energy_note": en.get("energy_note", ""),
                                    "price_change_2h_pct": round(price_change_pct2, 4),
                                    "price_change_24h_pct": round(pct24, 4),
                                    "quote_volume_24h": qv,
                                    "volume_last_5": vol_last5,
                                    "taker_buy_ratio": ratios,
                                    "is_binance_alpha": isalpha,
                                },
                                fingerprint=f"{sym}:{int(now_ts // 900)}",
                            )
                            issuper=False
...
async def cli():
    ...
    p_scan.add_argument("--limit", type=int, default=setting(KOL_SOURCE, "candidate_limit", 50), help="Max rows")
    p_scan.add_argument("--min-quote", type=float, default=setting(KOL_SOURCE, "min_quote_volume_24h", 20_000_000.0), help="Minimum 24h quote volume")
    p_scan.add_argument("--min-pct", type=float, default=setting(KOL_SOURCE, "min_change_24h_pct", 1.0), help="Minimum 24h change")
    ...
    p_run.add_argument("--limit", type=int, default=setting(KOL_SOURCE, "candidate_limit", 30), help="Max rows per tick")
    p_run.add_argument("--min-quote", type=float, default=setting(KOL_SOURCE, "min_quote_volume_24h", 20_000_000.0), help="Minimum 24h quote volume")
    p_run.add_argument("--min-pct", type=float, default=setting(KOL_SOURCE, "min_change_24h_pct", 1.0), help="Minimum 24h change")
    ...
    p_run.add_argument("--cooldown-minutes", type=int, default=setting(KOL_SOURCE, "source_cooldown_minutes", 1440), help="Source cooldown; publisher enforces one hour")

...
# 常量与默认配置
COINS = ["PLAY", "BEAT", "VELVET", "AIO"]
FEISHU_WEBHOOK = "https://open.feishu.cn/open-apis/bot/v2/hook/5c6197a4-6b2d-46ec-98e8-63b041af7c76"
...
def check_short_opportunity(
    ...
    vol_usd_threshold = 10_000_000 if use_15m_mode else 3_333_333
    ...
    cond_1 = c1_is_bear and (c1_vol >= compare_vol_1 * 1.5) and (c1_quote_vol >= vol_usd_threshold)
    cond_2 = c2_is_bear and (c2_vol >= compare_vol_2 * 1.5) and (c2_quote_vol >= vol_usd_threshold)
            ...
            if has_bull_candle:
                ...
                if max_bear_vol > 0:
                    ...
                    if len(sorted_vol_info) >= 2:
                        ...
                        cond1 = top1_is_bull and (top1_vol >= max_bear_vol * 0.75)
                        cond2 = top2_is_bull and (top2_vol >= max_bear_vol * 0.75)
                        ...
                        if cond1 and cond2:
                            ...
                            if len(oi_list) >= 5:
                                recent_oi = oi_list[-5:]
                                slope = calculate_slope(recent_oi)
                                if slope < 0 and trigger_drop_pct >= 0.40:
                                    oi_ok = False
...
async def send_feishu_alert(title: str, content: str):
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            logger.info(f"Send Feishu Alert: status={r.status_code}")
...
async def run_backtest_for_symbol(
    ...
    for i in range(len(base_klines)):
        ...
        if trigger_mode == "volume":
            ...
            if drop_pct_history >= 0.50 and not alerted_50:
                trigger_signal = True
                alerted_50 = True
                actual_drop_pct = drop_pct_history
                trigger_desc = f"历史最高价({current_high_history:.4f})盘中回调达 50%"
            elif drop_pct_history >= 0.40 and not alerted_40:
                trigger_signal = True
                alerted_40 = True
                actual_drop_pct = drop_pct_history
        ...
        T_ms = open_time
        Eval_T_ms = T_ms + 15 * 60 * 1000  # 观察评估时间：15分钟后

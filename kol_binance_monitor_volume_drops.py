...
KOL_SOURCE = configure_source("instant_drop_trigger")
...
STATE_FILE = setting(KOL_SOURCE, "state_file", "kol_monitor_volume_drops_state.json")
# 冷却时间：2小时 (7200秒)
ALERT_COOL_DOWN = setting(KOL_SOURCE, "source_cooldown_seconds", 7200)
# 回调幅度阈值：20%
DROP_THRESHOLD = setting(KOL_SOURCE, "drop_threshold", 0.20)
# 24小时成交额过滤阈值：8000w USDT
VOLUME_THRESHOLD = setting(KOL_SOURCE, "min_quote_volume_24h", 80_000_000.0)
# 检查间隔：10秒
CHECK_INTERVAL = setting(KOL_SOURCE, "check_interval_seconds", 10)
...
def append_triggered_signal(symbol: str, trigger_ts: int, trigger_price: float, drop_pct: float, trigger_desc: str):
    file_path = setting(KOL_SOURCE, "signal_file", "kol_triggered_signals.json")
    signals = []
        ...
        while True:
            ...
            try:
                ...
                for ticker in qualified_tickers:
                    ...
                    # 避免过快请求 K线 造成瞬时并发限频
                    await asyncio.sleep(setting(KOL_SOURCE, "per_symbol_delay_seconds", 0.1))
